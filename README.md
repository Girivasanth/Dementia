...
│   ├── games/                  # Cognitive evaluation modules
│   ├── speech/                 # Verbal task recording + upload
│   └── localization/           # Hindi, Tamil, Bengali strings
├── ml/
│   ├── gameplay_analysis/      # Behavioral indicator extraction
│   ├── speech_biomarkers/      # Acoustic feature pipeline
│   ├── mri_model/              # 3D CNN for MRI scan analysis
│   ├── oct_model/              # Retinal scan segmentation
│   ├── fusion/                 # Multimodal risk prediction
│   └── baseline/               # Personal calibration engine
├── backend/
│   ├── api/                    # FastAPI endpoints
│   ├── models/                 # Database schemas
│   └── scheduler/              # Assessment reminders + alerts
├── dashboard/                  # Neurologist web interface (React)
├── data/
│   └── anonymized_samples/     # De-identified sample records
├── docs/
│   ├── design_thinking/        # Stanford methodology documentation
│   └── clinical_protocol/      # Assessment design rationale
├── requirements.txt
└── README.md
```
...
git clone https://github.com/Girivasanth/cognisense
cd cognisense
...
# Run speech biomarker extraction on a sample
python speech_biomarkers/extract.py --input samples/speech_sample.wav

# Run MRI model inference
python mri_model/predict.py --scan samples/mri_sample.nii
